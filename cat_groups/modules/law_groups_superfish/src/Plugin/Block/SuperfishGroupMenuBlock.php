...
namespace Drupal\law_groups_superfish\Plugin\Block;
...
/**
 * Provides a "Superfish" block for Group Content Menus.
 *
 * @Block(
 *   id = "law_superfish_group_menu",
 *   admin_label = @Translation("Superfish group menu"),
 *   cache = -1,
 *   category = @Translation("Superfish group menus"),
 ...
 * )
 */
class SuperfishGroupMenuBlock extends GroupMenuBlock {
  ...
  /**
   * Constructs a new SuperfishGroupMenuBlock.
   *
  ...
  public function build() {
    ...
    // Set contextual links for the group menu.
  ...
  /**
   * Overrides \Drupal\group_content_menu\Plugin\Block\GroupMenuBlock::getMenuInstance().
   *
   * We need to manipulate the string key used to fetch the menu entity. The
   * group storage service assumes that you're using Group Content Menu module's
   * own native block plugin. We're not, so we pass in what that plugin ID would
   * have been.
   */
  public function getMenuInstance() {
    ...
    //// TODO: un-comment the @-var once on Group 2.x everywhere
    ...
    $plugin_id = str_replace('law_superfish_group_menu', 'group_content_menu', $this->pluginId);
    try {
