...
/**
 * @file
 * Contains \Drupal\Core\Field\Plugin\Field\FieldFormatter\FacultyProfileNameFull.
 */
 ...
 *
 * @FieldFormatter(
 *   id = "computed_faculty_profile_name_full",
 *   label = @Translation("Linked Profile Name Full"),
 *   field_types = {
 *     "computed_string",
...
class FacultyProfileNameFull extends ComputedPhpFormatterBase {
  ...
  public function formatItem($value, FieldItemInterface $item, $delta = 0, $langcode = NULL) {
    ...
    $display_name = $prefix . $first . $last . $grad;
    ...
    return '<span class="cf-linked-profile-name-full">' . $name_render . '</span>';
  }
}